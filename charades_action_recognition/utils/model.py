...
import Transformer.utils.Temporal_Mult as Temporal_Mult
import torch.nn as nn
import Transformer.utils.Image_Encoder as Image_Encoder
...
class Concat_model(nn.Module):
    def __init__(self,image_size,clip_size=64):
        super().__init__()
        self.temporal=Temporal_Mult.Temporal(image_size=image_size,clip_size=clip_size,patch_size=16).cuda()
        # self.resnet=model.cuda()
        self.ViVit=Image_Encoder.ViViT(image_size=image_size,patch_size=16,num_classes=157,num_frames=clip_size).cuda()
    ...
    def forward(self,rgb,origin_img):
        rgb_differ = self.temporal(origin_img)
        #,rgb_differ.cuda()
        rgb_results = self.ViVit(rgb_differ.cuda())
        # rgb=rgb.cuda()
        # rgb=rearrange(rgb,'b t c h w -> b c t h w')
        # rgb_results=self.resnet(rgb)
