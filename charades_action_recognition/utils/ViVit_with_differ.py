...
class ViViT(nn.Module):
    def __init__(self, image_size, patch_size, num_classes, num_frames, dim=192, depth=4, heads=3, pool='cls',
                 ...
                 emb_dropout=0., scale_dim=4, batch_size=16 ):
        super().__init__()
        ...
        self.mlp_head = nn.Sequential(
            ...
            nn.Conv1d(dim,num_classes,kernel_size=12,padding=0,stride=12)
        )


    def forward(self, x,rgb_differ):
        x=x+rgb_differ
        ...
        cls_temporal_tokens = repeat(self.temporal_token, '() n d -> b t n d', b=b,t=t)
        x = torch.cat((cls_temporal_tokens, x), dim=2)
        x = rearrange(x, 'b t n d -> (b t) n d')
        x = self.temporal_transformer(x)
        x = x.mean(dim=3) if self.pool == 'mean' else x[:, 0]
        x = rearrange(x, '(b t) ... -> b t ...', b=b,t=t)
